...
namespace AsyncTCPSocketServer
{
	...
	public class Server
	{
		...
		public static void AcceptCallback(IAsyncResult ar)
		{
			...
			handler.BeginReceive(state.headerBuffer, 0, StateObject.HeaderSize, 0, new AsyncCallback(RecvHeader), state);
		}

		public static void RecvHeader(IAsyncResult ar)
		{
			...
			if (bytesRead > 0)
			{
				...
				handler.BeginReceive(state.buffer, 0, (int)header.payloadSize, 0, new AsyncCallback(Recv), state);
			}
		}
		...
		private static void Send(Socket handler, string data)
		{
			...
			handler.BeginSend(buffer, 0, buffer.Length, 0, new AsyncCallback(OnSend), handler);
		}
